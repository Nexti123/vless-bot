        ...
        "Используйте меню ниже для управления подписками и кабинетом.", {
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '🤝 Партнерский кабинет', callback_data: 'partner_cabinet' }]
                ]
            }
        }
    );
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'my_keys') {
        ...
        try {
            ...
            bot.sendMessage(chatId, '❌ Ошибка при создании ключа на сервере. Убедитесь, что конфигурационный файл Xray существует.');
        }
    }
});
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                button { width: 100%; padding: 12px; background: #3b82f6; border: none; color: white; border-radius: 6px; font-weight: bold; cursor: pointer; }
            </style>
        </head>
        <body>
        ...
        usersHtml += `<tr>
            ...
            <td>
                <form method="POST" action="/admin/user/reset" style="display:inline;">
                    ...
                    <button type="submit" class="btn btn-sm" style="background:#f59e0b;">Обнулить</button>
                </form>
            </td>
        </tr>`;
    }
        ...
        keysHtml += `<tr>
            ...
            <td>
                <form method="POST" action="/admin/key/renew" style="display:inline; margin-right: 5px;">
                    ...
                    <button type="submit" style="background:#3b82f6;">+Дни</button>
                </form>
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                .card { background: #1e293b; padding: 20px; border-radius: 8px; flex: 1; }
                table { width: 100%; border-collapse: collapse; background: #1e293b; border-radius: 8px; overflow: hidden; }
                th, td { padding: 12px; border-bottom: 1px solid #334155; text-align: left; font-size: 14px; }
                th { background: #334155; }
                button { padding: 5px 10px; border: none; border-radius: 4px; color: white; cursor: pointer; }
        ...
        <body>
            <div class="container">
                <header>
                    <h1>⚡ Xray Admin Panel</h1>
                    <a href="/logout" style="background:#ef4444; padding: 8px 15px; border-radius: 4px;">Выйти</a>
                </header>
                <div class="stats">
                    <div class="card"><h3>Пользователи</h3><p style="font-size:20px;font-weight:bold;">${totalUsers}</p></div>
                    <div class="card"><h3>Активные ключи</h3><p style="font-size:20px;font-weight:bold;">${totalKeys}</p></div>
                    <div class="card"><h3>Общий доход</h3><p style="font-size:20px;font-weight:bold;">${sumTotalEarnings} руб.</p></div>
                </div>
                <div style="margin-bottom: 20px;">
                    <form method="POST" action="/admin/reset-all-earnings" onsubmit="return confirm('Обнулить заработок ВСЕХ пользователей?');">
                        <button type="submit" style="background:#f59e0b; padding: 10px 15px; font-weight:bold;">🔄 Обнулить всю систему заработка</button>
                    </form>
                </div>
                <h2>🔑 Ключи</h2>
                <table>
                    <tr><th>UUID</th><th>Название</th><th>Telegram ID</th><th>Срок</th><th>Статус</th><th>Действия</th></tr>
                    ${keysHtml || '<tr><td colspan="6" style="text-align:center;">Нет ключей</td></tr>'}
                </table>
                <h2 style="margin-top: 30px;">👥 Пользователи</h2>
                <table>
                    <tr><th>Telegram ID</th><th>Баланс</th><th>Реф. баланс</th><th>Всего заработок</th><th>Ключей</th><th>Действия</th></tr>
                    ${usersHtml || '<tr><td colspan="6" style="text-align:center;">Нет пользователей</td></tr>'}
                </table>
            </div>
        </body>
        </html>
...
app.post('/admin/key/renew', checkAuth, (req, res) => {
